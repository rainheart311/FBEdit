		...
		While TRUE
			...
			Select Case de.dwDebugEventCode
				Case EXCEPTION_DEBUG_EVENT
					Select Case de.u.Exception.ExceptionRecord.ExceptionCode
						Case EXCEPTION_BREAKPOINT
							If fExit=0 Then
								findthread(de.dwThreadId)
								gestbrk(Cast(UInteger,de.u.Exception.ExceptionRecord.ExceptionAddress))
							Else
								' Stop
					...
					If fContinue=DBG_EXCEPTION_NOT_HANDLED Then
						If de.u.Exception.dwFirstChance Then
							If fexit=0 Then
				...
				Case CREATE_THREAD_DEBUG_EVENT
					PutString("CREATE_THREAD_DEBUG_EVENT Thread=" & de.u.CreateThread.hThread)
					With de.u.CreateThread
						For i=0 To threadnb
				...
				Case EXIT_THREAD_DEBUG_EVENT
					For i=0 To threadnb
						If thread(i).threadid=de.dwThreadId Then
							PutString("EXIT_THREAD_DEBUG_EVENT ExitCode=" & de.u.ExitThread.dwExitCode & " Exitthread=" & thread(i).thread & " Returnthread=" & thread(i).threadret)
							thread(i).thread=0
				...
				Case EXIT_PROCESS_DEBUG_EVENT
					PutString("EXIT_PROCESS_DEBUG_EVENT ExitCode=" & de.u.ExitProcess.dwExitCode)
				...
				Case LOAD_DLL_DEBUG_EVENT
					buffer=""
					GetModuleFileName(de.u.LoadDll.lpBaseOfDll,@buffer,256)
					PutString("LOAD_DLL_DEBUG_EVENT " & buffer)
				Case UNLOAD_DLL_DEBUG_EVENT
					buffer=""
					GetModuleFileName(de.u.UnloadDll.lpBaseOfDll,@buffer,256)
					PutString("UNLOAD_DLL_DEBUG_EVENT " & buffer)
				Case OUTPUT_DEBUG_STRING_EVENT
					nln=de.u.DebugString.nDebugStringLength
					If nln>255 Then
						nln=255
					EndIf
					lret=ReadProcessMemory(dbghand,de.u.DebugString.lpDebugStringData,@buffer,nln,0)
					PutString(@buffer)
				Case RIP_EVENT
					PutString("RIP_EVENT")
			End Select
