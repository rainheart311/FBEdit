...
Function CreateEdit(ByVal sFile As String) As HWND
	...
	hCtl=ah.hred
	tpe=FileType(sFile)
	If Cast(Boolean,tpe=2) And Cast(Boolean,(GetKeyState(VK_CONTROL) And &H80)=0) And fNoResMode=FALSE Then
		hTmp=IsResOpen
		If hTmp Then
	...
	Else
		...
		ElseIf tpe=1 Then
		    ...
		    SetWindowLong(hTmp,GWLP_USERDATA,2)
		Else
	    ...
	    lpOldParEditProc=Cast(Any Ptr,SetWindowLongPtr(hTmp,GWLP_WNDPROC,Cast(Integer,@ParEditProc)))
	...
	Do While TRUE
		If SendMessage(ah.htabtool,TCM_GETITEM,i,Cast(Integer,@tci)) Then
			...
			Select Case As Const nType
				...
				Case 3
					If lpTABMEM->hedit<>ah.hres Then
					    x=GetWindowLong(lpTABMEM->hedit,GWLP_USERDATA)
