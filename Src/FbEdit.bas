...
'/
'#Define _EXCEPTIONS_INIT_CODE_ 	'very important has to be placed before the following include and only 1 time
'								at the first code source file of project, multi .o compiled files
'								or main file .bas if others sources files are included
'#include once "inc\Exception_Management_Win.bi"

#Include Once "inc\FbEdit.bi"

#Include Once "inc\Language.bi"
#Include Once "src\IniFile.bas"
#Include Once "src\CodeComplete.bas"
#Include Once "src\Misc.bas"
#Include Once "src\FileIO.bas"
#Include Once "src\TabTool.bas"
#Include Once "src\Goto.bas"
#Include Once "src\Toolbar.bas"
#Include Once "src\About.bas"
#Include Once "src\KeyWords.bas"
#Include Once "src\Make.bas"
#Include Once "src\MenuOption.bas"
#Include Once "src\TabOptions.bas"
#Include Once "src\BlockInsert.bas"
#Include Once "src\Project.bas"
#Include Once "src\ExternalFile.bas"
#Include Once "src\PathOpt.bas"
#Include Once "src\Export.bas"
#Include Once "src\Find.bas"
#Include Once "src\HexFind.bas"
#Include Once "src\Resource.bas"
#Include Once "src\DebugOpt.bas"
#Include Once "src\CreateTemplate.bas"
#Include Once "src\Addins.bas"
#Include Once "src\Language.bas"
#Include Once "inc\fbeditini.bi"
	...
	If fTimer Then
		...
		If fTimer=0 Then
			...
			If ah.hred<>0 And ah.hred<>ah.hres Then
				...
				SendMessage(ah.hsbr,SB_SETTEXT,1,Cast(LPARAM,@buffer)) '修
				...
				Else
					buffer=""
				EndIf
				SendMessage(ah.hsbr,SB_SETTEXT,4,Cast(LPARAM,@buffer))               '修
				If SendMessage(ah.hred,REM_GETMODE,0,0) And MODE_OVERWRITE Then
					SendMessage(ah.hsbr,SB_SETTEXT,2,Cast(LPARAM,StrPtr("")))        '修改
				Else
					SendMessage(ah.hsbr,SB_SETTEXT,2,Cast(LPARAM,StrPtr("  INS")))   '修改
				EndIf
			EndIf
			UpdateAllTabs(3)
			UpdateAllTabs(4)
...
Function MainDlgProc(ByVal hWin As HWND,ByVal uMsg As UINT,ByVal wParam As WPARAM,ByVal lParam As LPARAM) As Integer
	Dim As Long id,x,y,bm,hgt,wdt,tbhgt,twt,prjht,prht,i
	Dim lret As Integer
	Dim rect As RECT
	Dim rect1 As RECT
	Dim chrg As CHARRANGE
	...
	Dim hCtl As HWND
	Dim lfnt As LOGFONT =(-14,0,0,0,400,FALSE,FALSE,FALSE,0,0,0,0,0,"宋体")  
	Dim tci As TCITEM
	Dim lpTABMEM As TABMEM Ptr
	...
	Select Case uMsg
		Case WM_INITDIALOG
			If ah.hwnd = NULL Then ah.hwnd=hWin       '如果是空，则保存
			If ah.hwnd <> hWin Then Exit Function     '如果不是本窗体的事件，不处理，退出
			ad.lpFBCOLOR=@fbcol
			ad.lpWINPOS=@wpos
			For id=1 To 15
			...
			sbParts(0)=80               '32/64位编译器选择
			sbParts(1)=240				'鼠标位置
			sbParts(2)=280				'INS
			sbParts(3)=440				'编译方式 - 控制台，GUI，DLL，LIB
			sbParts(4)=-1				'鼠标所在函数
			SendMessage(ah.hsbr,SB_SETPARTS,5,Cast(Integer,@sbParts(0)))
			SetWindowText(ah.hsbr,@"  FB Win 32Bit")
		    ...
		    lpOldTabToolProc=Cast(Any Ptr,SetWindowLongptr(ah.htabtool,GWLP_WNDPROC,Cast(Integer,@TabToolProc)))
			' Handle of output window
			ah.hout=GetDlgItem(hWin,IDC_OUTPUT)
		    lpOldOutputProc=Cast(Any Ptr,SetWindowLongptr(ah.hout,GWLP_WNDPROC,Cast(Integer,@OutputProc)))
			' Handle of immediate window
			ah.himm=GetDlgItem(hWin,IDC_IMMEDIATE)
		    lpOldImmediateProc=Cast(Any Ptr,SetWindowLongPtr(ah.himm,GWLP_WNDPROC,Cast(Integer,@ImmediateProc)))
			...
			If Abs(edtfnt.size) Mod 2 = 1 Then edtfnt.size += 1 '
			...
			lfnt.lfCharSet=134'lnrfnt.charset
			...
			lfnt.lfHeight=-11
			lfnt.lfCharSet=134'edtfnt.charset
			lstrcpy(lfnt.lfFaceName,StrPtr("宋体"))
			lfnt.lfItalic=0
			...
			tci.pszText=@buff
			SendMessage(ah.htab,TCM_INSERTITEM,999,Cast(Integer,@tci))
			'buff="Resource"'GetInternalString(IS_RESOURCE)
			'tci.pszText=@buff
			'SendMessage(ah.htab,TCM_INSERTITEM,999,Cast(Integer,@tci))
			'Project browser
			ah.hprj=GetDlgItem(hWin,IDC_TRVPRJ)
		    lpOldProjectProc=Cast(Any Ptr,SetWindowLongPtr(ah.hprj,GWLP_WNDPROC,Cast(Integer,@ProjectProc)))
			...
			'SendMessage(hResource,TVM_SETIMAGELIST,TVSIL_NORMAL,Cast(Integer,ah.himl)) '
		    ...
		    lpOldCCProc=Cast(Any Ptr,SetWindowLongPtr(ah.hcc,GWLP_WNDPROC,Cast(Integer,@CCProc)))
			...
			'SendMessage(hResource,WM_SETFONT,Cast(Integer,ah.hToolFont),0)
		    ...
		    lpOldSplashProc=Cast(Any Ptr,SetWindowLongPtr(GetDlgItem(hWin,IDC_IMGSPLASH),GWLP_WNDPROC,Cast(Integer,@SplashProc)))
			SetFocus(hWin)
			frhex=FR_DOWN
			ttmsg.szType="M"
            ...
            Select Case ptnmhdr->idFrom
            	Case IDC_STATUSBAR
                    ...
                    If ptnmhdr->code = NM_CLICK Then 
                        ...
                        If lpnm->dwItemSpec = 0 Then
                        	Select Case SelFbcType
                        		Case 1
                        			SelFbcType = 2
                        			SetWindowText(ah.hsbr,@"  FB Win 64Bit")
                        		Case 2
                        			SelFbcType = 1
                        			SetWindowText(ah.hsbr,@"  FB Win 32Bit")
                        		Case Else
				...
				Else
					If GetWindowLong(ah.hred,GWL_ID)=IDC_CODEED Then
						...
						If lpRASELCHANGE->Line<>nLastLine Then
							ShowWindow(ah.htt,SW_HIDE)
							HideList()
						    If GetWindowLongPtr(ah.hred,GWLP_USERDATA)=1 Then
								' Must be parsed
								SetWindowLongptr(ah.hred,GWLP_USERDATA,2)
						    EndIf
						EndIf
						If lpRASELCHANGE->fchanged Then
							...
							ElseIf lpRASELCHANGE->Line<nLastLine Then
								nLastLine+=1
							EndIf
						    If GetWindowLongPtr(ah.hred,GWLP_USERDATA)=0 Then
								SetWindowLongPtr(ah.hred,GWLP_USERDATA,1)
						    EndIf
							' Set comment block definition
...
Function WinMain(ByVal hInst As HINSTANCE,ByVal hPrevInst As HINSTANCE,ByVal lpCmdLine As LPSTR,ByVal CmdShow As Integer) As Integer
	...
	CreateDialogParam(hInst,Cast(ZString Ptr,IDD_MAIN),NULL,@MainDlgProc,NULL)
...
'{	Program start
	...
	If hRAEditDll AndAlso hRAPropertyDll AndAlso hRACodeCompleteDll AndAlso hRAResEdDll AndAlso hRAFileDll AndAlso hRAGridDll AndAlso hRAHexEdDll Then
