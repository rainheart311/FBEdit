...
Sub BrowseFolder(ByVal hWin As HWND,ByVal nID As Integer)
	...
	bri.lpfn = Cast(BFFCALLBACK,@BrowseCallbackProc)
	' get path   
...
Sub UpdateProjectFileName(ByVal sOldFile As LPCTSTR,ByVal sNewFile As LPCTSTR)
	Dim nInx As Integer
	Dim nMiss As Integer
	Dim sItem As ZString*260
...
Function TrvAddNode(ByVal hPar As HTREEITEM,ByVal lpPth As ZString Ptr,ByVal nImg As Integer,ByVal lParam As Integer) As HTREEITEM
	...
	If hPar Then
		...
		'If nProjectGroup<>1 Then '按目录分组也需要自动展开
...
Sub RefreshProjectTree
	...
	tvs.hParent=hPar
	tvs.lpfnCompare = Cast(PFNTVCOMPARE,@TVCompare)
	tvs.lParam=0
...
Function OpenProject() As Integer
	...
	SelFbcType = GetPrivateProfileInt(StrPtr("Project"),StrPtr("FBCType"),1,@ad.ProjectFile) '设置默认是32Bit
	...
	'ShowWindow(hProject,SW_HIDE) '这里必须隐藏窗体，否则打开不能正常显示工程目录
...
Function ApiOptionProc(ByVal hWin As HWND,ByVal uMsg As UINT,wParam As WPARAM,lParam As LPARAM) As Integer
	...
	Select Case uMsg
		Case WM_INITDIALOG
		    ...
		    lpOldApiListProc=Cast(Any Ptr,SetWindowLongptr(hLst,GWLP_WNDPROC,Cast(Integer,@ApiListProc)))
			s=ApiFiles
			While Len(s)
				sItem=GetTextItem(s)
...
Function ProjectOptionDlgProc(ByVal hWin As HWND, ByVal uMsg As UINT, ByVal wParam As WPARAM, ByVal lParam As LPARAM) As Integer
	...
	Select Case uMsg
		Case WM_INITDIALOG
			...
			buff = "None"
			SendDlgItemMessage(hWin,IDC_FBCTYPE,CB_ADDSTRING,0,Cast(Integer,@buff))
			buff = "FreeBASIC Windows - 32Bit"
			SendDlgItemMessage(hWin,IDC_FBCTYPE,CB_ADDSTRING,0,Cast(Integer,@buff))
			buff = "FreeBASIC Windows - 64Bit"
			SendDlgItemMessage(hWin,IDC_FBCTYPE,CB_ADDSTRING,0,Cast(Integer,@buff))
			SendDlgItemMessage(hWin,IDC_FBCTYPE,CB_SETCURSEL,SelFbcType,0)
			'
		Case WM_CLOSE
			EndDialog(hWin, 0)
			'
		Case WM_COMMAND
			Select Case LoWord(wParam)
				Case IDOK
					...
					SelFbcType = SendDlgItemMessage(hWin,IDC_FBCTYPE,CB_GETCURSEL,0,0)  '32位编译器，64位编译器选择
					WritePrivateProfileString(StrPtr("Project"),StrPtr("FBCType"),Str(SelFbcType),@ad.ProjectFile)
					Select Case SelFbcType
						Case 1
            			    SetWindowText(ah.hsbr,@"  FB Win 32Bit")
						Case 2
