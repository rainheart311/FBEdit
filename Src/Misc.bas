...
Sub AddApiFile(ByVal sFile As LPCTSTR,ByVal nType As Integer)
...
Function GetFileName(ByVal sFile As LPCTSTR,ByVal fExt As Boolean) As String
	Dim x As Integer
	Dim sItem As ZString*260
	...
	Select Case uMsg
		Case WM_CHAR
			If SendMessage(hPar,REM_GETMODE,0,0)=0 Then
				' Mode Normal
				hPar=GetParent(hWin)
				If wParam=VK_ESCAPE Then                       '关闭提示列表
					ShowWindow(ah.htt,SW_HIDE)
					HideList()
					Return 0
				ElseIf wParam=VK_TAB Or wParam=VK_RETURN Then  '代码提示用Tab和回车键上屏
				...
				If SendMessage(hPar,REM_GETWORDGROUP,0,0)=0 Then
					...
					If wParam = VK_SPACE Or wParam = VK_TAB Or wParam = Asc("(") Or wParam = Asc(",") Or wParam = VK_BACK Or fmessagelist <> 0 Or fenumlist <> 0 Then
					  ...
					  TT:
						...
						If SendMessage(ah.hpr,PRM_GETTOOLTIP,TT_NOMATCHCASE Or TT_PARANTESES,Cast(LPARAM,VarPtr(tt))) And (InStr(buff,"(")<>0 Or InStr(buff," ")<>0) And lx=-1 Then
					...
					TestCaseConvert(hPar,wParam)
					SendMessage(hPar,EM_EXGETSEL,0,Cast(LPARAM,@prechrg))
					If Cast(Boolean,wParam=Asc(".") Or wParam=Asc(">")) And fconstlist=TRUE Then
						HideList
					EndIf
				TestUpdate:
					If Cast(Boolean,IsWindowVisible(ah.hcc)) Or Cast(Boolean,fconstlist) Then
		...
		Case WM_KILLFOCUS
	        ...
	        If GetWindowLong(hPar,GWLP_WNDPROC)=1 Then
				' Must be parsed
				SetWindowLong(hPar,GWLP_WNDPROC,2)
			EndIf
		...
		Do While LnSt<=LnEn
			...
			For tmp=0 To 127
				If n=nmin Then
					...
					chrg.cpMax=chrg.cpMin
					SendMessage(ah.hred,EM_EXSETSEL,0,Cast(LPARAM,@chrg))
					buffer=Space(1) '准备一个字符的空间
					buffer = char   '赋值给buffer
					SendMessage(ah.hred,EM_REPLACESEL,TRUE,Cast(LPARAM,@buffer)) '替换注释符号
					If lnCnt>0 Or chrg.cpMin<ochrg.cpMax Then
						ochrg.cpMax+=1
						If LnCnt=0 And chrg.cpMin<=ochrg.cpMin Then
							ochrg.cpMin+=1
...
Sub ShowProjectTab()
    Select Case SendMessage(ah.htab,TCM_GETCURSEL,0,0)
	    Case 0
		' File browser
...
Sub CenterOwner(ByVal hWin As HWND)
	...
	Dim rect As RECT
	Dim rect1 As RECT
    hPar=Cast(HWND,GetWindowLongPtr(hWin,GWLP_HWNDPARENT))
	If hPar=0 Then
