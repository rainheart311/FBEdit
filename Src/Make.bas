Function Replace(ByRef sMainString As String,ByRef sMatchPattern As String,ByRef sReplaceWith As String) As String
    Dim i As Integer 
    Dim s As String 
...
Sub GetMakeOption()
	...
	If fProject Then
		...
		If Len(ad.smake) Then
			...
			SendMessage(ah.hsbr,SB_SETTEXT,3,Cast(Integer,@sText)) 'ÐÞ
	...
	Select Case SelFbcType
		Case 1
			ad.smake = Replace(ad.smake,"fbc -","fbc32 -")
		Case 2
			ad.smake = Replace(ad.smake,"fbc -","fbc64 -")
		Case Else
	End Select
End Sub
...
Function MakeThreadProc(ByVal Param As ZString Ptr) As Integer
	...
	Else
		...
		stupinfo.hStdOutput=makeinf.hwr
		stupinfo.hStdError=makeinf.hwr
		' Create process
		stupinfo.dwFlags=STARTF_USESHOWWINDOW
		stupinfo.wShowWindow=SW_SHOWNORMAL
		If CreateProcess(NULL,@buff,NULL,NULL,FALSE,NULL,NULL,NULL,@stupinfo,@makeinf.pInfo)=0 Then
...
Function MakeRun(ByVal sFile As LPCTSTR,ByVal fDebug As Boolean) As Integer
...
Function MakeProc(ByVal Param As Integer) As Integer
	...
	Else
		stupinfo.cb = SizeOf(STARTUPINFO)
		GetStartupInfo(@stupinfo)
		stupinfo.hStdOutput=hwr
		stupinfo.hStdError=hwr
		stupinfo.dwFlags=STARTF_USESHOWWINDOW+STARTF_USESTDHANDLES
		stupinfo.wShowWindow=SW_HIDE
		' Create process
		lret=CreateProcess(NULL,@buff,NULL,NULL,TRUE,NULL,NULL,NULL,@stupinfo,@pinfo)
		If lret=0 Then
			' CreateProcess failed
			CloseHandle(hrd)
			CloseHandle(hwr)
