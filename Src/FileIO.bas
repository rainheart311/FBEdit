Declare Sub AddAProjectFile(ByVal sFile As string,ByVal fModule As Boolean,ByVal fCreate As Boolean)
...
Function GetFileMem(ByVal sFile As LPCTSTR) As HGLOBAL
	Dim tci As TCITEM
...
Function ParseFile(ByVal hWin As HWND,ByVal hEdit As HWND,ByVal sFile As String) As Integer
	...
	If fParse Then
		If hEdit Then
			...
			hMem=MyGlobalAlloc(GMEM_FIXED Or GMEM_ZEROINIT,nlen+1)
			SendMessage(hEdit,WM_GETTEXT,nlen,Cast(Integer,hMem))
		    SetWindowLong(hEdit,GWLP_USERDATA,0)
		Else
			hFile=CreateFile(sFile,GENERIC_READ,FILE_SHARE_READ,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0)
			If hFile<>INVALID_HANDLE_VALUE Then
				nlen=GetFileSize(hFile,NULL)+1
...
Sub ReadTextFile(ByVal hWin As HWND,ByVal hFile As HANDLE,ByVal lpFilename As ZString Ptr)
	...
	If FileType(szItem)=1 Then
		...
		UpdateAllTabs(3)
		If fProject<>FALSE And Cast(Boolean,Len(ad.resexport)) Then
...
Sub BackupFile(ByVal szFileName As LPCTSTR,ByVal nBackup As Integer)
...
Sub SaveResourceNames(ByVal szFileName As String)
	...
	sFileName = Mid(szFileName,1,InStrRev(szFileName,Any "\/")) & "Resource.bi"
	nSize = GetWindowTextLength(ah.hout) + 1
	sTemp = Space(nSize)
	GetWindowText(ah.hout,ByVal StrPtr(sTemp),nSize)
	sTemp = RTrim(sTemp, Chr(0))
	SendMessage(ah.hraresed,PRO_EXPORTNAMES,1,Cast(Integer,ah.hout))
	nSize = GetWindowTextLength(ah.hout) + 1
	sSave = Space(nSize)
	GetWindowText(ah.hout,ByVal StrPtr(sSave),nSize)
	sSave = RTrim(sSave, Chr(0))
	SetWindowText(ah.hout,StrPtr(sTemp))
	Open sFileName For Output As #f
        Put #f,,sSave
    Close #f 
    If IsProjectFile(sFileName) Then
    	OpenTheFile(sFileName,FALSE)
    Else
        AddAProjectFile(sFileName,FALSE,FALSE)
    EndIf 
End Sub
